...
class RowChecker:
    ...
    def _validate_pair(self, row):
        if row[self.first_col] and row[self.second_col]:
            ...
            assert (
                Path(row[self.first_col]).suffixes
                == Path(row[self.second_col]).suffixes
            ), f"FASTQ pairs must have the same file extensions."
        else:
...
def parse_args(args=None):
    ...
    parser.add_argument(
        "file_in", metavar="FILE_IN", type=Path, help="Input sample sheet."
    )
